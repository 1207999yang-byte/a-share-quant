...
st.set_page_config(
    page_title="A股量化助手 V3.6",
    page_icon="📈",
    layout="wide",
    initial_sidebar_state="expanded",
)

st.title("📈 A股量化助手 V3.6")
st.caption("自动行情 + V3.6代码修复 + 多源容错 + 股票搜索 + 多因子量化评分 + K线 + 买入/观望/回避 + 10万元模拟账户")
...
def load_quote(code):
    ...
    # 2) 雪球个股行情：补齐 PE/PB 等估值字段，避免只有腾讯收盘价时估值因子变成 None
    try:
        fn = getattr(ak, "stock_individual_spot_xq", None)
        if fn:
            xq = fn(symbol=market_suffix(code).replace(".", ""))
            if xq is not None and not xq.empty and {"item", "value"}.issubset(xq.columns):
                d = dict(zip(xq["item"].astype(str), xq["value"]))
                price = to_num(d.get("现价"))
                if pd.isna(price):
                    price = to_num(d.get("最新价"))
                return {
                    "代码": code,
                    "名称": str(d.get("名称", code)),
                    "最新价": price,
                    "涨跌幅": to_num(d.get("涨幅")),
                    "市盈率-动态": to_num(d.get("市盈率(动)")),
                    "市净率": to_num(d.get("市净率")),
                }
    except Exception as e:
        errors.append("雪球行情: " + str(e)[:120])
    # 3) 腾讯历史行情：最后一道行情备用源
    try:
...
def load_financial(code):
    """V3.6 财务多源：重点兼容同花顺新版“长表”结构，并用新浪宽表兜底。"""
    if ak is None:
        return pd.DataFrame()
    code = clean_code(code)
    records = []

    def add_standard(source, row_date, roe=np.nan, rev_yoy=np.nan, profit_yoy=np.nan,
                     debt=np.nan, ocf_sales=np.nan):
        records.append({
            "REPORT_DATE": pd.to_datetime(row_date, errors="coerce"),
            "ROEJQ": to_num(roe),
            "TOTALOPERATEREVETZ": to_num(rev_yoy),
            "PARENTNETPROFITTZ": to_num(profit_yoy),
            "ZCFZL": to_num(debt),
            "JYXJLYYSR": to_num(ocf_sales),
            "FIN_SOURCE": source,
        })

    # 1) 新浪财务指标：字段稳定、直接返回“每个报告期一行”
    try:
        fn = getattr(ak, "stock_financial_analysis_indicator", None)
        if fn:
            df = fn(symbol=code, start_year=str(max(2020, datetime.now().year - 6)))
            if df is not None and not df.empty:
                date_col = next((c for c in ["日期", "报告期", "REPORT_DATE"] if c in df.columns), None)
                if date_col:
                    df = df.copy()
                ...
                add_standard(
                    "新浪财务指标",
                    row.get("_date", row.get(date_col) if date_col else pd.NaT),
                    roe=col("加权净资产收益率(%)", "净资产收益率(%)", "净资产报酬率(%)"),
                    rev_yoy=col("主营业务收入增长率(%)", "营业收入增长率(%)"),
                    profit_yoy=col("净利润增长率(%)"),
                    debt=col("资产负债率(%)"),
                    ocf_sales=col("经营现金净流量对销售收入比率(%)"),
                )
    except Exception:
        pass

    # 2) 同花顺新版：长表 metric_name/value/yoy；成长率应取 yoy，而不是 value
    try:
        ...
        if fn:
            ...
            if df is not None and not df.empty and "metric_name" in df.columns:
                x = df.copy()
                x["report_date"] = pd.to_datetime(x.get("report_date"), errors="coerce")
                x["metric_name"] = x["metric_name"].astype(str)
                latest_date = x["report_date"].max()
                x = x[x["report_date"] == latest_date] if pd.notna(latest_date) else x
                def metric(keys, use_yoy=False):
                    names = x["metric_name"]
                    for keyset in keys:
                        mask = names.map(lambda z: all(k.lower() in z.lower() for k in keyset))
                        if mask.any():
                            r = x.loc[mask].iloc[-1]
                            return r.get("yoy") if use_yoy else r.get("value")
                    return np.nan
                add_standard(
                    "同花顺财务指标",
                    latest_date,
                    roe=metric([["净资产收益率"], ["ROE"]]),
                    rev_yoy=metric([["营业总收入"] , ["营业收入"]], use_yoy=True),
                    profit_yoy=metric([["归母净利润"], ["净利润"]], use_yoy=True),
                    debt=metric([["资产负债率"]]),
                    ocf_sales=metric([["经营现金流"] , ["经营活动现金流"]], use_yoy=False),
                )
    except Exception:
        pass

    # 3) 新浪关键指标宽表：用于部分老版本 AKShare
    try:
        ...
        if fn:
            ...
            if df is not None and not df.empty and "指标" in df.columns:
                ...
                if date_cols:
                    ...
                    def abstract_metric(keys):
                        names = df["指标"].astype(str)
                        for ks in keys:
                            mask = names.map(lambda z: all(k.lower() in z.lower() for k in ks))
                            if mask.any():
                                return df.loc[mask, latest].iloc[-1]
                        return np.nan
                    add_standard(
                        "新浪关键指标",
                        pd.to_datetime(latest, format="%Y%m%d", errors="coerce"),
                        roe=abstract_metric([["净资产收益率"], ["ROE"]]),
                        rev_yoy=np.nan,
                        profit_yoy=np.nan,
                        debt=abstract_metric([["资产负债率"]]),
                        ocf_sales=np.nan,
                    )
    except Exception:
        pass
    ...
    # 对同一日期/不同来源取“每个字段第一个有效值”，尽量拼出完整的一行
    out = {"REPORT_DATE": pd.NaT, "ROEJQ": np.nan, "TOTALOPERATEREVETZ": np.nan,
           "PARENTNETPROFITTZ": np.nan, "ZCFZL": np.nan, "JYXJLYYSR": np.nan,
           "FIN_SOURCE": ""}
    sources = []
    for r in sorted(records, key=lambda z: (pd.Timestamp.min if pd.isna(z["REPORT_DATE"]) else z["REPORT_DATE"])):
        if pd.isna(out["REPORT_DATE"]) and pd.notna(r["REPORT_DATE"]):
            out["REPORT_DATE"] = r["REPORT_DATE"]
        for k in ["ROEJQ", "TOTALOPERATEREVETZ", "PARENTNETPROFITTZ", "ZCFZL", "JYXJLYYSR"]:
            if pd.isna(out[k]) and pd.notna(r[k]):
                out[k] = r[k]
        if r.get("FIN_SOURCE"):
            sources.append(r["FIN_SOURCE"])
    out["FIN_SOURCE"] = "、".join(dict.fromkeys(sources))
    valid = [k for k in ["ROEJQ", "TOTALOPERATEREVETZ", "PARENTNETPROFITTZ", "ZCFZL", "JYXJLYYSR"] if pd.notna(out[k])]
...
with tabs[0]:
    ...
    if "analysis" in st.session_state:
        ...
        fin_view = pd.DataFrame([
            {"指标": "ROE", "数值": fmt_num(a.get("ROE"), "%")},
            {"指标": "营收同比", "数值": fmt_num(a.get("营收同比"), "%")},
            {"指标": "净利润同比", "数值": fmt_num(a.get("净利润同比"), "%")},
            {"指标": "资产负债率", "数值": fmt_num(a.get("资产负债率"), "%")},
            {"指标": "经营现金流/营收", "数值": fmt_num(a.get("经营现金流/营收"), "%")},
        ])
        st.dataframe(fin_view, use_container_width=True, hide_index=True)
...
### 常见问题
**1. 行情获取失败**
可能是公开数据源限流、网络波动或接口变更。V3.6 会依次尝试批量行情、雪球个股行情和腾讯历史行情；若仍失败，稍后重试即可。
